...
def _all_exact_rows(spec, args, exact_pids):
    ...
    exact_dir = _exact_dir(spec, args)
    stats = _filter_pids(_read_csvs(_pid_frame_paths(exact_dir, ".csv", exact_pids)), exact_pids)
    if not stats.empty:
        ...
        rows.append(
            {
                ...
                "method": "Exact",
                "setting": "exact",
                "width": _value(stat, "initial_width"),
...
def _all_restricted_rows(spec, args, variant, exact_pids):
    ...
    if not stats.empty:
        stat_cols = [
            column
            for column in ("pid", "total_time", "initial_width")
...
def parse_args():
    ...
    parser.add_argument(
        "--all-results-csv",
        type=Path,
        default=Paths.results / "knapsack_all_results.csv",
