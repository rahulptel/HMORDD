...
class KnapsackProblem(Problem):
    def __init__(self, instance_data):
        ...
        self.n_cons = instance_data.get("n_cons", 1) # Default to 1 if not typically set, but utils shows it is read.

        self.W = np.array(instance_data["weight"]).reshape(1, -1)
        self.V = np.array(instance_data["value"])
        self.C = instance_data["capacity"]

        super().__init__(
            n_var=self.n_vars,
            n_obj=self.n_objs,
            n_ieq_constr=1, # Knapsack usually has 1 constraint: sum(w*x) <= C
            xl=0,
            xu=1,
            vtype=bool,
        )
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        # self.W: "weight": [int, ...] -> 1D array of weights
        out["G"] = (np.dot(X, self.W.T) - self.C)
...
class Runner(BaseRunner):
    ...
    def _run_nsga2(self, instance_data, pid, pop_size: int, run_time: int, run_seed: int):
        problem = KnapsackProblem(instance_data)
