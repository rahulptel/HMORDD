#!/usr/bin/env python3
"""Builds META-Farm case definitions for knapsack NSGA-II runs."""
...
CUTOFFS: Iterable[str] = ("restrict",)
INSTANCES_PER_CASE = 10
SPLIT = "test"
INST_SEED = 7
POP_SIZES: Iterable[int] = (100, 500)
...
def build_command(
    n_objs: int, n_vars: int, pid_start: int, pid_end: int, cutoff: str, pop_size: int
) -> str:
    """Create the command line executed by META-Farm for one case."""
    pythonpath = SRC_PY.as_posix()
    return (
        f"PYTHONPATH={pythonpath}:$PYTHONPATH "
        f"python -m hmordd.knapsack.run_nsga2 nsga2.pop_size={pop_size} "
            ...
            for pop_size in POP_SIZES:
                for case_idx in range(cases):
                    ...
                    command = build_command(
                        n_objs, n_vars, pid_start, pid_end, cutoff, pop_size
                    )
